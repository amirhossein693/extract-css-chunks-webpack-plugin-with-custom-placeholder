        ...
        if (renderedModules.length > 0) {
          result.push({
            ...
            filenameTemplate: ({
              ...
              let filename = this.options.moduleFilename(chunkData);
              if (REGEXP_PAGEDIR.test(filename)) {
                filename = filename.replace(REGEXP_PAGEDIR, 'ltr')
              }
              return filename;
            },
            pathOptions: {
      ...
      compilation.chunkTemplate.hooks.renderManifest.tap(pluginName, (result, {
        ...
        if (renderedModules.length > 0) {
          result.push({
            render: () => this.renderContentAsset(compilation, chunk, renderedModules, compilation.runtimeTemplate.requestShortener),
            filenameTemplate: () => {
              let filename = this.options.chunkFilename;
              if (REGEXP_PAGEDIR.test(filename)) {
                filename = filename.replace(REGEXP_PAGEDIR, 'ltr')
              }
              return filename;
